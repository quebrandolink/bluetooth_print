...
class MethodChannelBluetoothScanner implements IBluetoothScanner {
  ...
  final StreamController<List<BluetoothDevice>> _scanResultsController = StreamController.broadcast();
  ...
  final StreamController<void> _stopScanController = StreamController<void>.broadcast();
  final StreamController<MethodCall> _methodCallController = StreamController.broadcast();
  ...
  Stream<BluetoothDevice> scan({Duration timeout = const Duration(seconds: 5)}) {
    ...
    final sub = _methodCallController.stream.where((m) => m.method == 'ScanResult').map((m) => BluetoothDevice.fromJson(Map<String, dynamic>.from(m.arguments))).listen((device) {
      final addr = device.address?.toUpperCase().trim();
      if (addr == null || addr.isEmpty) return;
      final index = _results.indexWhere((e) => e.address?.toUpperCase().trim() == addr);
      if (index == -1) {
        _results.add(device);
        _scanResultsController.add(List.from(_results));
        controller.add(device);
      } else if (_results[index].name != device.name || _results[index].type != device.type) {
